...
#####################################################################################
# here from ChatGPT for SwinUNet
...
class SwinUNet(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.encoder.features[0][0] = nn.Conv2d(in_chns, 96, kernel_size=4, stride=4, padding=0, bias=False)
        
        self.up1 = UpBlock_SwinUNet(384, 192, 96, dropout_p=0.0) ## better for 128*128 input images
        self.up2 = UpBlock_SwinUNet(96, 48, 48, dropout_p=0.0)   ## better for 128*128 input images
        self.up3 = UpBlock_SwinUNet(48, 24, 24, dropout_p=0.0)
        self.up4 = UpBlock_SwinUNet(24, 16, 16, dropout_p=0.0)
        
        self.out_conv = nn.Conv2d(16, class_num, kernel_size=1)
    
    def forward(self, x):
        features = self.encoder.features(x)
        
        x4 = features[-1]  # Last feature map
        x3 = features[-2]  # Second last feature map
        x2 = features[-3]
        x1 = features[-4]
        
        # Ensure all feature maps have (batch, channels, H, W) shape
        if len(x4.shape) == 3:  
            x4 = x4.unsqueeze(-1).unsqueeze(-1)  # Add height/width
        if len(x3.shape) == 3:
...
class UpBlock_SwinUNet(nn.Module):
    def __init__(self, in_channels1, in_channels2, out_channels, dropout_p=0.0):
        super(UpBlock_SwinUNet, self).__init__()
        self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.conv = ConvBlock_SwinUNet(in_channels2 * 2, out_channels, dropout_p)
...
class ConvBlock_SwinUNet(nn.Module):
    def __init__(self, in_channels, out_channels, dropout_p):
        super(ConvBlock_SwinUNet, self).__init__()
        self.conv_conv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1),
            nn.BatchNorm2d(out_channels),
